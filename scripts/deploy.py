...
def deploy(addons_dir: Path, dry_run: bool) -> int:
    ...
    if dest.resolve() == REPO_ROOT.resolve():
        print(
            ...
            "Deploy is intended for a separate dev clone (e.g. eso_dev). "
            "Use make-zip.py from this repo instead."
        )
        return 1
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Deploy ActivityFinderPlus to ESO AddOns.")
    parser.add_argument(
        "--addons-dir",
        type=Path,
